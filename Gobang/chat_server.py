...
# ==============================================================================
                ...
                self.msgTime.append(time.strftime('%d.%m.%y,%H:%M', time.localtime()))
                # ---- end of your code --- #
                ...
                for g in the_guys:
                    ...
                    mysend(to_sock, json.dumps({"action":"exchange", "message": message, "from": "[" + from_name+ "]"}))
...
# ==============================================================================
                ...
                # IMPLEMENTATION
                # ---- start your code ---- #
                poemLineLst = self.sonnet.get_poem(int(msg["target"]))
                poem = '\n'.join(poemLineLst)
                # ---- end of your code --- #
...
# ==============================================================================
                ...
                for name, index in self.indices.items():
                    searchOutcome = str()
                    result = self.indices[name].search(msg["target"])
                    for re in result:
                        print(re[0])
                        searchOutcome += '(' + self.msgTime[re[0]] +') ' + name + " : " + re[1] + '\n'
                        #searchOutcome +=  name + " : " + re[1] + '\n'
                    search_rslt += searchOutcome
                # ---- end of your code --- #
                mysend(from_sock, json.dumps(
...
#---------------------------------------------------------------------------
                ...
                if len(the_guys) == 1: 
                    g = the_guys.pop()
                    to_sock = self.logged_name2sock[g]
                    mysend(to_sock, json.dumps({"action": "quit_game", "msg": "Peer left the game."}))

            elif msg["action"] == "restart_game":
                from_name = self.logged_sock2name[from_sock]
                the_guys = self.gameGroup.list_me(from_name)
                to_name = the_guys[1]
                to_sock = self.logged_name2sock[to_name]
